	...
	public class MsgPackWriter
	{
		...
		public void Write (string x, byte[] buf, bool highProbAscii)
		{
            ...
            Encoder encoder = _encoder;
            {
				...
				WriteRawHeader (encoder.GetByteCount (strData,0,x.Length, true));
				int str_len = x.Length;
                int str_index = 0;
				int convertedChars, bytesUsed;
				bool completed = true;
				while (str_len > 0 || !completed) {
					encoder.Convert (strData, str_index,str_len, buf,0,buf.Length, false, out convertedChars, out bytesUsed, out completed);
					_strm.Write (buf, 0, bytesUsed);
					str_len -= convertedChars;
					str_index += convertedChars;
				}
			}
#else
            // アンマネージ版。これだと iOS ビルドできず
            unsafe{
                Encoder encoder = _encoder;
                fixed (char *pstr = x)
                    fixed (byte *pbuf = buf) {
